...
static pmvec3_t rgv3tStuckTable[54];
...
void PM_InitTextureTypes()
{
	...
	static pmboolean bTextureTypeInit = false;
...
void PM_UpdateStepSound(void)
{
		 ...
		 (speed >= velwalk || !pmove->flTimeStepSound) )
	{
		...
		pmove->flTimeStepSound = (int)(pmove->flTimeStepSound + flduck);  // slower step time if ducking
...
================
*/
pmboolean PM_AddToTouched(pmtrace_t tr, pmvec3_t impactvelocity)
{
	int i;
...
==================
*/
int PM_ClipVelocity(pmvec3_t in, pmvec3_t normal, pmvec3_t out, float overbounce)
{
	float backoff;
	float change;
	float angle;
...
int PM_FlyMove(void)
{
	...
	float d;
	int numplanes;
	pmvec3_t planes[MAX_CLIP_PLANES];
	pmvec3_t primal_velocity, original_velocity;
	pmvec3_t new_velocity;
	int i, j;
	pmtrace_t trace;
	pmvec3_t end;
...
==============
*/
void PM_Accelerate(pmvec3_t wishdir, float wishspeed, float accel)
{
	int i;
...
void PM_AirAccelerate(pmvec3_t wishdir, float wishspeed, float accel)
{
	int i;
...
pmboolean PM_CheckWater()
{
	...
	if ( cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT )
	{
		...
		if ( (truecont <= CONTENTS_CURRENT_0) && (truecont >= CONTENTS_CURRENT_DOWN) )
		{
			...
			static pmvec3_t current_table[] = {{1, 0, 0}, {0, 1, 0}, {-1, 0, 0}, {0, -1, 0}, {0, 0, 1}, {0, 0, -1}};
...
=================
*/
int PM_GetRandomStuckOffsets(int nIndex, int server, pmvec3_t offset)
{
...
void PM_LadderMove(pmphysent_t* pLadder)
{
	...
	if ( trace.fraction != 1.0 )
	{
		...
		else
		{
			...
			{
				pmvec3_t velocity, perp, cross, lateral, tmp;
				float normal;
...
============
*/
pmtrace_t PM_PushEntity(pmvec3_t push)
{
	pmtrace_t trace;
	pmvec3_t end;
...
{
	pmvec3_t vecStart, vecEnd;
	pmvec3_t flatforward;
	pmvec3_t flatvelocity;
	float curspeed;
	pmtrace_t tr;
	int savehull;
...
===============
*/
float PM_CalcRoll(pmvec3_t angles, pmvec3_t velocity, float rollangle, float rollspeed)
{
	float sign;
	float side;
	float value;
...
=============
*/
void PM_DropPunchAngle(pmvec3_t punchangle)
{
	float len;
...
void PM_CreateStuckTable(void)
{
	...
	memset(rgv3tStuckTable, 0, 54 * sizeof(pmvec3_t));
